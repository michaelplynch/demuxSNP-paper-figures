...
tenx_path<-c("/data/demuxSNP/data/processed/pbmc_data_4x_out/outs/filtered_feature_bc_matrix")
barcodes_path<-c("/data/scratch/nextflow/data_sub/barcodes_merged_p.tsv")
lookup_path<-c("/data/scratch/nextflow/data_sub/lookup_table_doublets_pbmc_p_40pc.tsv")
...
sce
ngroup=4
z<-matrix(0,nrow=4,ncol=dim(sce)[2])
...
size_sig=rep(10,4)
size_bg=rep(7,4)
mu_sig=c(400,500,200,400)
mu_bg=c(90,100,60,80)
