...
chr<-GenerateCellHashingCalls(counts,
                              methods=c("htodemux","bff_raw","bff_cluster","dropletutils"),
                              doTSNE = FALSE)

seurat_high<-CreateSeuratObject(counts=counts,assay="HTO")
seurat_high<-NormalizeData(seurat_high,normalization.method ="CLR")
seurat_high<-HTODemux(seurat_high)

#sce_high<-SingleCellExperiment()

chr$gt<-truth
library(caret)
methods=c("htodemux","bff_cluster","bff_raw")

for (i in seq_along(methods[1])) {
  m<-methods[i]
  data_ch=chr[[m]]
  ref_ch=chr[['gt']]
...
table(data_ch,ref_ch)

table(chr$htodemux,seurat_high$hash.ID)
