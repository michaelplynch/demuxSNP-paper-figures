---
title: "Figure-4-application"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Figure-4-application}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r setup}
library(demuxSNPpaperfigures)
library(CiteFuse)
library(viridisLite)
```

````{r}

colors <- structure(viridis(n = 3), names = c("-1", "0", "1"))
colors_hashtags <- structure(dittoColors()[1:8], names = c("Hashtag1", "Hashtag2", "Hashtag3","Hashtag4","Hashtag5","Hashtag6","Doublet","Negative"))
````

````{r}

load("../data/sce_app.rda")

````


````{r}

library(SIBERG)
altExp(sce_app,"HTO")<-as(altExp(sce_app,"HTO"), "SingleCellExperiment")
c<-counts(altExp(sce_app,"HTO"))[2,]
sb<-fitNB(c)

sb
hist(c)
hist(log(c+1))
sig1<-rnbinom(10000,mu=sb[1],size=1/sb[3])
sig2<-rnbinom(3000,mu=sb[2],size=1/sb[4])
hist(log(c(sig1,sig2)+1))
hist(c(sig1,sig2))
# dat <- rnbinom(100, mu=1000, size=1/0.2)
# fitNB(y=dat)

````

````{r}

library(scuttle)
df <- perCellQCMetrics(sce_app,
                       subsets=list(Mito=grep("^MT",rownames(sce_app))),
                       use_altexps=FALSE)
sce_app$lib_size<-df$sum
sce_app$percent_mito<-df$subsets_Mito_percent

````


````{r}
library(CiteFuse)
sce_app<-normaliseExprs(sce=sce_app,altExp_name = "HTO",transform="log")
sce_app<-crossSampleDoublets(sce_app)

library(Seurat)
seurat_app<-as.Seurat(sce_app,data=NULL)
seurat_app <- NormalizeData(seurat_app, assay = "HTO", normalization.method = "CLR")
seurat_app <- HTODemux(seurat_app, assay = "HTO", positive.quantile = 0.99)

table(seurat_app$hash.ID)
table(sce_app$doubletClassify_between_label,seurat_app$hash.ID)
#s2<-seurat_app[,seurat_app$nCount_RNA>1000 & seurat_app$percent_mito<10]
#s2<-HTODemux(s2, assay= "HTO")
#table(s2$hash.ID)
````



````{r}
df2<-as.data.frame(df[seurat_app$hash.ID=="Negative",])
df2$high_quality<-df2$sum>1500 & df2$detected>1000
library(ggpubr)

plots<-ggscatterhist(df2,x="sum",y="detected",color = "high_quality", title = "Quality metrics distributions for 'Negative' group")
plots$sp <- plots$sp +
    geom_hline(yintercept = 1000, linetype = "dashed", color = "blue") +
  geom_vline(xintercept = 1500, linetype = "dashed", color = "red") 
plots
ggpar(plots,xscale="log10",yscale="log10",xlab=("library size"),ylab=c("detected genes"))
high_qual<-df2$sum>1500 & df2$detected>1000
ggpar(plots,xlab=("library size"),ylab=c("detected genes"))
high_qual<-df2$sum>1500 & df2$detected>1000
table(high_qual)


````



````{r fig.height=2,fig.width=7}
barplot(table(high_qual))
df_qual<-data.frame(table(high_qual))
df_qual$high_quality<-"high_quality"
ggbarplot(df_qual,x="high_quality",y="Freq", fill="high_qual",xlab=c(""),label = TRUE, lab.col = "white", lab.pos = "in",lab.vjust = c(0,0),lab.hjust = c(6.5,1.5),title = c("Numbers of high and low quality cells in HTODemux Negative group")) + rotate()+ylab("# cells")+xlab("")+rremove("y.text")
````

 alluvial plots https://enblacar.github.io/SCpubr-book-v1/13-AlluvialPlot.html

````{r}



````


````{r fig.height=6,fig.width=15}
sample<-seurat_app
library(dplyr)
library(SCpubr)
p1 <- SCpubr::do_AlluvialPlot(sample = sample,
                            first_group = "hash.ID",
                            last_group = "doubletClassify_between_label",
                            flip=TRUE)



p1
table(seurat_app$hash.ID,seurat_app$doubletClassify_between_label)
````



````{r}
sce<-sce_app
altExp(sce,"HTO")<-as(altExp(sce,"HTO"), "SingleCellExperiment")
library(Matrix)
library(demuxSNP)
mat<-as.matrix(readMM('../data/v_outs_full.mtx'))
sce<-add_snps(sce,mat=mat)
sce<-high_conf_calls(sce)
sce<-reassign(sce)

sce$hash.ID<-seurat_app$hash.ID
soup<-read.table(file="../data/clusters.tsv", sep='\t',header=TRUE)
soup$assignment[soup$status=="doublet"]<-"Doublet"
soup$assignment[soup$status=="unassigned"]<-"unassigned"
sce$soup<-soup$assignment

table(sce$knn)
table(sce$hash.ID)
table(sce$soup)

table(sce$soup,sce$knn)

sce$soup_rc<-recode(sce$soup,
                    "0"="Hashtag1",
                    "1"="Hashtag5",
                    "2"="Hashtag6",
                    "3"="Hashtag3",
                    "4"="Hashtag4",
                    "5"="Hashtag2")

table(sce$soup_rc,sce$knn)


````

# alluvial plot

````{r fig.width=12,fig.height=4}


library(ggalluvial)
df_all<-data.frame(soup=sce$soup_rc,demuxSNP=sce$knn,htodemux=sce$hash.ID)
df_allrs<-reshape2::melt(table(sce$soup_rc,sce$knn,sce$hash.ID))
colnames(df_allrs)<-c("souporcell","demuxSNP","HTODemux","ncells")
df_allrs$demuxSNP<-factor(df_allrs$demuxSNP,levels=c(paste("Hashtag",1:6,sep=""),"Doublet"))
ggplot(data = df_allrs,
       aes(axis1 = souporcell, axis2 = demuxSNP, axis3 = HTODemux, y = ncells)) +
  geom_alluvium(aes(fill = demuxSNP)) +
  geom_stratum() +
  geom_text(stat = "stratum",
            aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Souporcell", "demuxSNP","HTODemux"),
                   expand = c(0.15, 0.05)) +
  scale_fill_manual(values=dittoColors()[1:8]) +
   coord_flip()

````


````{r}

table(sce$hash.ID)
table(sce$soup)
````


````{r}
library(demuxmix)
counts_low=as.matrix(counts(altExp(sce,"HTO")))
dmm<-demuxmix(hto=counts_low,model="naive")
pAcpt(dmm) <- 0
dmmlabels<-dmmClassify(dmm)
dmmlabels$HTO[dmmlabels$Type=="negative"]<-"Negative"
dmmlabels$HTO[dmmlabels$Type=="multiplet"]<-"Doublet"
split<-c(dmmlabels$Prob<0.9)

hto_jnorm<-NormalizeData(counts_low,normalization.method = "CLR")
df_j<-data.frame(t(hto_jnorm))
#plot_hashtag(df_j,split=split)

````
# Negative SNP heatmap

````{r}
library(ComplexHeatmap)
sce_negative<-sce[,sce$hash.ID=="Negative" & c(sce$percent_mito<15 | sce$lib_size>1000)]
ha = HeatmapAnnotation(demuxSNP = sce_negative$knn, 
                       HTODemux = sce_negative$hash.ID,
                       annotation_legend_param = list(
                          demuxSNP = list(
                              title = "Assignment"
                          )
                      ),
                      col=list(demuxSNP=colors_hashtags,HTODemux=colors_hashtags),
                      show_legend = c(FALSE,FALSE)
)
hm<-Heatmap(counts(altExp(sce_negative,"SNP")),
        cluster_rows = FALSE,
        column_split=sce_negative$knn,
        show_column_dend = FALSE,
        show_column_names = FALSE,
        cluster_column_slices = FALSE,
        column_title = "Cells",
        column_title_side = "bottom",
        row_title = "SNPs",
        show_row_names = FALSE,
        col = colors,
        top_annotation = ha,
        heatmap_legend_param = list(title="SNP status",at=c(-1,0,1),labels=c("Absent","No reads","Present")),
        )

draw(hm,
    column_title = "HTODemux high quality Negative SNP profiles split by demuxSNP call",
    padding = unit(c(2, 15, 2, 2), "mm")
)
````

# Negative distance matrix

````{r}
sce<-sce[,sce$percent_mito<15 & sce$lib_size>1000]
snps_neg<-counts(altExp(sce,"SNP"))

d<-dist(t(snps_neg))
dd<-as.matrix(d)
dim(d)
hashtags<-rownames(altExp(sce,"HTO"))
ddd<-matrix(0,6,6)
for (i in 1:6) {
  hashtag_neg<-hashtags[i]
  for (j in 1:6) {
    hashtag_knn<-hashtags[j]
    c1<-c(sce$hash.ID=="Negative" & sce$knn==hashtag_neg)
    c2<-c(sce$train==TRUE & sce$knn==hashtag_knn)
    ddd[i,j]<-mean(dd[c1,c2])
  }
}
ddd
library(ComplexHeatmap)
library(circlize)
col_fun<-colorRamp2(c(0,3),c("white","brown"))
colnames(ddd)<-hashtags
rownames(ddd)<-hashtags
Heatmap(log(ddd),cluster_rows=FALSE,cluster_columns=FALSE,col=col_fun)

snps<-counts(altExp(sce,"SNP"))
library(geosphere)
centroid<-c()
combs1<-paste(sce$hash.ID, sce$knn[])
levels1<-grep("^Negative",unique(combs1),value=TRUE)
combs2<-paste(sce$train,sce$knn)
levels2<-grep("^TRUE",unique(combs2))
centroid1<-matrix(0,92,6)
for (i in c(1,3,4,5,6)) {
  hashtag<-hashtags[i]
  centroid1[,i]<-rowMeans(snps[,sce$hash.ID=="Negative" & sce$knn==hashtag])
}
colnames(centroid1)<-paste("Hashtag",c(1,2,3,4,5,6))
centroid1
centroid2<-matrix(0,92,6)
for (i in c(1,3,4,5,6)) {
  hashtag<-hashtags[i]
  centroid2[,i]<-rowMeans(snps[,sce$train==TRUE & sce$knn==hashtag])
}
colnames(centroid2)<-paste("Hashtag", c(1:6))
centroid2

c_full<-cbind(centroid1,centroid2)

````

````{r fig.height=5,fig.width=5.5}
mat<-as.matrix(dist(t(c_full)))
mat<-mat[-c(2,8),-c(2,8)]
mat_sub<-mat[1:5,6:10]
col_fun<-colorRamp2(c(0,max(mat)),c("white","brown"))
hm<-Heatmap(mat_sub,
        cluster_rows = FALSE,
        cluster_columns = FALSE,
        col=col_fun,column_title = "High confidence cells",
        row_title = "Negative reassigned cells",
        heatmap_legend_param = list(title = "Euclidean distance",direction = "horizontal"),
        cell_fun = function(j, i, x, y, w, h, col) { # add text to each grid
        grid.text(round(mat_sub[i, j],2), x, y)},
        column_names_rot = 0,
        column_names_centered = TRUE,
        show_heatmap_legend = FALSE
    )
draw(hm,heatmap_legend_side = "bottom")
````

# Hashtag2 SNP heatmap

````{r}

sce_hashtag2<-sce[,sce$hash.ID=="Hashtag2"]
colors_hashtags <- structure(dittoColors()[1:8], names = c("Hashtag1", "Hashtag2", "Hashtag3","Hashtag4","Hashtag5","Hashtag6","Doublet","Negative"))

ha = HeatmapAnnotation(demuxSNP = sce_hashtag2$knn, 
                       HTODemux = sce_hashtag2$hash.ID,
                       annotation_legend_param = list(
                          demuxSNP = list(
                              title = "Assignment"
                          )
                      ),
                      col=list(demuxSNP=colors_hashtags,HTODemux=colors_hashtags),
                      show_legend = c(FALSE,FALSE)
)

hm<-Heatmap(counts(altExp(sce_hashtag2,"SNP")),
        cluster_rows = FALSE,
        column_split=sce_hashtag2$knn,
        show_column_dend = FALSE,
        show_column_names = FALSE,
        cluster_column_slices = FALSE,
        row_title = "SNPs",
        column_title="Cells",
        column_title_side = "bottom",
        show_row_names = FALSE,
        col = colors,
        heatmap_legend_param = list(title="SNP status",at=c(-1,0,1),labels=c("Absent","No reads","Present")),
        top_annotation = ha
        )


draw(hm,
    column_title = "HTODemux high quality Hashtag2 SNP profiles split by demuxSNP call",
    padding = unit(c(2, 15, 2, 2), "mm"),
    heatmap_legend_side="right",
    annotation_legend_side="right"
)
````

# Hashtag 2 binary distance hist and matrix


````{r}

sce_hashtag2<-sce[,sce$hash.ID=="Hashtag2"]
table(sce_hashtag2$knn)
sce_hashtag2$knn

snps_all<-counts(altExp(sce,"SNP"))

d<-dist(t(snps_all))
dd<-as.matrix(d)
dim(d)
hashtags<-rownames(altExp(sce,"HTO"))
ddd<-matrix(0,6,6)
for (i in 1:6) {
  hashtag_neg<-hashtags[i]
  for (j in 1:6) {
    hashtag_knn<-hashtags[j]
    c1<-c(sce$hash.ID=="Hashtag2" & sce$knn==hashtag_neg)
    c2<-c(sce$train==TRUE & sce$knn==hashtag_knn)
    ddd[i,j]<-mean(dd[c1,c2])
  }
}
ddd
library(ComplexHeatmap)
library(circlize)
col_fun<-colorRamp2(c(0,3),c("white","brown"))
colnames(ddd)<-hashtags
rownames(ddd)<-hashtags
Heatmap(log(ddd),cluster_rows=FALSE,cluster_columns=FALSE,col=col_fun)

snps<-counts(altExp(sce,"SNP"))
library(geosphere)
centroid<-c()
combs1<-paste(sce$hash.ID, sce$knn[])
levels1<-grep("^Hashtag2",unique(combs1),value=TRUE)
combs2<-paste(sce$train,sce$knn)
levels2<-grep("^TRUE",unique(combs2))
centroid1<-matrix(0,92,6)
for (i in c(2,3,4,5)) {
  hashtag<-hashtags[i]
  centroid1[,i]<-rowMeans(snps[,sce$hash.ID=="Hashtag2" & sce$knn==hashtag])
}
colnames(centroid1)<-paste("Hashtag",c(1,2,3,4,5,6))
centroid1
centroid2<-matrix(0,92,6)
for (i in c(2,3,4,5)) {
  hashtag<-hashtags[i]
  centroid2[,i]<-rowMeans(snps[,sce$train==TRUE & sce$knn==hashtag])
}
colnames(centroid2)<-paste("Hashtag", c(1:6))
centroid2

c_full<-cbind(centroid1,centroid2)

````



````{r fig.height=4.5,fig.width=5}
mat<-as.matrix(dist(t(c_full)))
#mat<-mat[-c(2,8),-c(2,8)]
mat_sub<-mat[2:5,8:11]
col_fun<-colorRamp2(c(0,max(mat)),c("white","brown"))
hm<-Heatmap(mat_sub,
        cluster_rows = FALSE,
        cluster_columns = FALSE,
        col=col_fun,column_title = "High confidence cells",
        row_title = "Hashtag2 reassigned cells",
        heatmap_legend_param = list(title = "Euclidean distance",direction = "horizontal"),
        cell_fun = function(j, i, x, y, w, h, col) { # add text to each grid
        grid.text(round(mat_sub[i, j],2), x, y)},
        column_names_rot = 0,
        column_names_centered = TRUE,
        show_heatmap_legend = FALSE
    )
draw(hm,heatmap_legend_side = "bottom")
````

````{r}

snps_hashtag2<-t(counts(altExp(sce,"SNP")[,sce$hash.ID=="Hashtag2" & sce$lib_size>1500]))
dfull<-dist(snps_hashtag2)
hist(dfull)
snps_bin<-snps_hashtag2
snps_bin[snps_bin==c(-1)]<-0
library(vegan)
vdist<-vegdist(snps_bin,method="jaccard",binary=TRUE)
hist(vdist)

vdist<-vegdist(snps_hashtag2,method="jaccard",binary=FALSE)
hist(log(vdist+1))


library(ade4)
db<-dist.binary(data.frame(snps_bin),method = 2)
dbm<-data.frame(binary_distance=matrix(db))

gghistogram(dbm,x='binary_distance',xlab=c("Binary distance (simple matching coefficient)"))
````

````{r}
library(vegan)
i=1
hashtags<-paste("Hashtag",1:6,sep="")
plots<-list()
for (i in seq_along(hashtags)) {
ht<-hashtags[i]
snps<-t(counts(altExp(sce,"SNP")[,sce$hash.ID==ht & sce$lib_size>1500]))
snps_bin<-snps
snps_bin[snps==c(-1)]<-0

vdist<-vegdist(snps_bin,method="jaccard",binary=TRUE)
#hist(vdist)

dbm<-data.frame(binary_distance=matrix(vdist))
#plot(Heatmap(t(snps_bin),cluster_rows = FALSE,show_column_names = FALSE,show_row_names = FALSE))
gg<-gghistogram(dbm,x='binary_distance',xlab=c("Binary distance (jaccard)"),fill=dittoColors()[i],alpha = 1)
#plot(gg)
#gg2<-ggpar(gg, tickslab = FALSE)
gg2<-gg+rremove('y.text')
plot(gg2)
  plots[[i]]<-gg2
}
````

````{r fig.height=7,fig.width=6.5}

grid.arrange(grobs=plots,nrow=3,top=textGrob("Within-group SNP distance distributions for HTODemux", gp=gpar(fontsize=18,font=8)))
````

````{r}
hashtags<-c("Hashtag1","Hashtag2","Hashtag3","Hashtag4","Hashtag5","Hashtag6")
for (i in seq_along(hashtags)) {
  ht<-hashtags[i]
  print(ht)
  match<-sum((as.character(sce$hash.ID)[sce$hash.ID==ht]==as.character(sce$knn)[sce$hash.ID==ht])/length(sce$hash.ID[sce$hash.ID==ht]))
  print(match)
}

````


````{r}

sce
Heatmap(counts(altExp(sce[,sce$hash.ID=="Hashtag4"],"SNP")),
        column_split = sce$hash.ID[sce$hash.ID=="Hashtag4"],
        cluster_rows = FALSE,
        show_column_names = FALSE,
        column_title_rot = 45)
````


````{r}

lgd = Legend(at = c("Hashtag1","Hashtag2","Hashtag3","Hashtag4","Hashtag5","Hashtag6","Doublet","Negative"), title = "Assignment", legend_gp = gpar(fill = dittoColors()[1:8]),nrow=2)
draw(lgd,test="")


col_fun<-colorRamp2(c(0,10),c("white","brown"))
lgd = Legend(col_fun = col_fun, title = "Euclidean Distance", at = c(0, 10),
    labels = c("Similar", "Dissimilar"),
    direction = "horizontal")
draw(lgd,test="")
````


# SNP data vs libsize

````{r}
sce<-logNormCounts(sce)
x<-colSums(logcounts(sce))
y<-colSums(mat!=0)
df_snpqual<-data.frame(libsize=x,snps=y)
plot(x,y)
ggscatterhist(df_snpqual,x='libsize',y='snps')
````


