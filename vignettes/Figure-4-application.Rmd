...
sce<-reassign_balanced(sce,k=20,d=100,n=200)
...
library(ggalluvial)
df_all<-data.frame(soup=sce$soup_rc,demuxSNP=sce$knn_balanced,htodemux=sce$hash.ID)
df_allrs<-reshape2::melt(table(sce$soup_rc,sce$knn_balanced,sce$hash.ID))
...
library(geosphere)
centroid<-c()
combs1<-paste(sce$hash.ID, sce$knn_balanced[])
levels1<-grep("^Negative",unique(combs1),value=TRUE)
combs2<-paste(sce$train,sce$knn_balanced)
levels2<-grep("^TRUE",unique(combs2))
centroid1<-matrix(0,92,6)
for (i in c(1:6)) {
  hashtag<-hashtags[i]
  centroid1[,i]<-rowMeans(snps[,sce$hash.ID=="Negative" & sce$knn_balanced==hashtag])
}
...
for (i in c(1:6)) {
  hashtag<-hashtags[i]
  centroid2[,i]<-rowMeans(snps[,sce$train==TRUE & sce$knn_balanced==hashtag])
}
...
ha = HeatmapAnnotation(demuxSNP = sce_hashtag2$knn_balanced, 
...
for (i in c(2,3,4,5)) {
  hashtag<-hashtags[i]
  centroid1[,i]<-rowMeans(snps[,sce$hash.ID=="Hashtag2" & sce$knn_balanced==hashtag])
}
colnames(centroid1)<-paste("Hashtag",c(1:6),"ds",sep="")
#centroid1
centroid2<-matrix(0,92,6)
...
for (i in seq_along(hashtags)) {
  ht<-hashtags[i]
  print(ht)
  match<-sum((as.character(sce$hash.ID)[sce$hash.ID==ht]==as.character(sce$knn_balanced)[sce$hash.ID==ht])/length(sce$hash.ID[sce$hash.ID==ht]))
  print(match)
}
