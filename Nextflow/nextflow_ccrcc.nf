...
process DEMUXSNP {
    ...
    input:
    tuple val(key), val(doublets), path(bam), path(bai), path(barcodes), val(pcdoub), path(lookup), path(merged_bcs), val(seed)
    ...
    Rscript !{projectDir}/templates/demuxSNP_nf_pre.R !{params.common_variants} "sce.rdata" !{key} !{doublets} !{seed}
    vartrix_linux --bam !{bam} \
        --cell-barcodes !{barcodes} \
        --fasta !{params.ref} \
        --threads 5 \
        --vcf "demuxSNP_!{key}_!{doublets}_!{seed}/vcf_sub.vcf"
    Rscript !{projectDir}/templates/demuxSNP_nf_post.R sce.rdata out_matrix.mtx !{barcodes} !{key} !{doublets} !{seed}
    end=`date +%s`
    runtime=$((end-start))
...
workflow {
    ...
    SOUP(test_ch)
    //VIREO(runseeds_ch)
    DEMUXSNP(test_ch.combine(LOOKUP.out.lookup,by:0).combine(merged_bcs_ch,by:0).combine(seed_ch))
}
